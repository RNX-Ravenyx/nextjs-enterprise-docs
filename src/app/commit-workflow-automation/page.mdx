...
This setup allows for a variety of commit types, providing flexibility while maintaining consistency. Additionally, the configuration can be customized, such as enforcing JIRA ticket numbers in commit messages, to further align with project management requirements. Example:
...
module.exports = {
  ...
  rules: {
    'type-enum': [
      2,
      'always',
      /^(feat|fix|perf|refactor|docs|chore|style|test)(\(\w+\))?: (OW-\d+) - .+$/,
    ],
  },
}
```
